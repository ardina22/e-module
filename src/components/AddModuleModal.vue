...
import { PlusIcon, DocumentPlusIcon, RectangleStackIcon } from '@heroicons/vue/24/outline'
...
<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
    <div class="bg-white/90 backdrop-blur-md rounded-lg w-full max-w-3xl shadow-xl relative flex flex-col max-h-[90vh]">
      <!-- Header -->
      ...
      <div class="overflow-y-auto px-6 py-4 space-y-6 grow">
        ...
        <form @submit.prevent="addModule" class="space-y-5">
          ...
          <div>
            ...
            <div class="relative">
              <input v-model="form.label" type="text" required placeholder="e.g. Introduction"
              ...
              <DocumentPlusIcon class="w-5 h-5 absolute right-3 top-1/2 -translate-y-1/2 text-gray-400" />
            </div>
          </div>

          <!-- Type toggle -->
          <div class="flex items-center gap-2">
            <input type="checkbox" v-model="form.isFile"
              class="form-checkbox text-blue-600 border-gray-300 focus:ring-2 focus:ring-blue-400" />
            <label class="text-sm font-medium text-gray-700">
              This is a file (has content)
            </label>
          </div>
